  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-2xl shadow-sm shadow-violet-100/40 p-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Financeiro e Contratos</h1>
          <p className="text-slate-500 text-sm mt-0.5">Receitas, despesas e mensalidades</p>
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          {activeTab === 'contracts' && (
            <button onClick={() => setIsContractModalOpen(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-violet-600 hover:bg-violet-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors shadow-sm shadow-violet-200">
              <Plus className="h-4 w-4" /> Novo Contrato
            </button>
          )}
          {activeTab === 'expenses' && (
            <button onClick={() => setIsRecordModalOpen(true)} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-rose-600 hover:bg-rose-700 text-white px-4 py-2.5 rounded-xl text-sm font-semibold transition-colors">
              <Plus className="h-4 w-4" /> Nova Despesa
            </button>
          )}
          {activeTab === 'overview' && (
            <button onClick={handleAIAnalysis} disabled={analyzing} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-violet-600 to-indigo-600 hover:opacity-90 text-white px-5 py-2.5 rounded-xl text-sm font-semibold shadow-sm transition-all disabled:opacity-60">
      ...
      {/* Tabs */}
      <div className="bg-white rounded-2xl shadow-sm shadow-violet-100/40 p-1.5 flex gap-1 overflow-x-auto">
        {tabs.map(tab => (
          <button
            key={tab.id}
            ...
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl text-sm font-semibold transition-all whitespace-nowrap min-w-[110px] ${
              activeTab === tab.id ? 'bg-violet-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 hover:bg-slate-50'
            }`}
          >
        ...
        <div className="space-y-5">
          {aiInsight && (
            <div className="bg-violet-50 border border-violet-100 rounded-2xl p-5">
              <h3 className="font-bold text-violet-900 mb-2 flex items-center gap-2"><Sparkles className="h-4 w-4" /> Insights da IA</h3>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              { label: 'Saldo Líquido', value: `R$ ${balance.toLocaleString('pt-BR')}`, color: balance >= 0 ? 'text-slate-800' : 'text-rose-600', icon: Wallet, bg: 'bg-violet-50', ic: 'text-violet-600' },
              ...
              <div key={k.label} className="bg-white rounded-2xl shadow-sm shadow-violet-100/40 p-5">
              ...
              <tbody className="divide-y divide-slate-50">
                {contracts.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <button className="text-xs font-semibold text-violet-600 hover:text-violet-800 hover:underline">Ver PDF</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              <div key={k.label} className="bg-white rounded-2xl shadow-sm shadow-violet-100/40 p-5 flex items-center gap-4">
          ...
          >
            ...
            <form onSubmit={handleContractSubmit} className="p-5 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" className="flex-1 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl font-semibold text-sm transition-colors">Criar Contrato</button>
              </div>
            </form>
          </div>
        </div>
